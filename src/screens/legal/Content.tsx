...
import { getLegalData } from 'src/redux/features/legalSlice'
...
const Content = () => {
  const { data, status } = useAppSelector(state => state.legalReducer)
  const dispatch = useAppDispatch()

  useEffect(() => {
    if (status === StateRequest.EMPTY) dispatch(getLegalData())
  }, [status])
