...
const ArtistByIdDesktop = () => {
  ...
  const formattedDescription = artist?.description_en
    ?.replace(/<\/p><p>&nbsp;<\/p><p>/g, '\n\n')
    .replace(/<p>|<\/p>/g, '')
    .replace(/&amp;/g, '&')
    .replace(/&lt;/g, '<')
  ...
  return (
    <div className='relative flex px-10 py-14 w-full'>
      {/* Flecha izquierda */}
      <button
        className='absolute top-1/4 left-8 transform -translate-y-1/2 text-4xl'
      ...
      {/* Flecha derecha */}
      <button
        className='absolute top-1/4 right-8 transform -translate-y-1/2 text-4xl'
        onClick={() => console.log('4')}
      >
        <TextIcon
