    ...
    return (
        <div className={`relative h-screen ${styles.artistListContainer} text-center ${styles.customCursor}`} style={{ flexBasis: "28.125%" }}>
            <div
                ...
                className={`overflow-y-scroll h-full ${styles.scrollContainer} ${isDragging ? 'drag noSelect' : ''}`}
            ...
            >
               ...
               ${clickedArtist === item.id ? 'text-yellow-lime-app'
                                                    : selectedArtist?.id === item.id ? 'text-white opacity-100'
            ...
            {isDragging && !clickOnButton && (
                ...
                >
                    <span className="inter opacity-100 desk:text-[22px] absolute top-[18%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white">△</span>
                    <span className="inter opacity-100 desk:text-[22px] absolute bottom-[18%] left-1/2 transform -translate-x-1/2 translate-y-1/2 text-white">▽</span>
                </div>
            )}
